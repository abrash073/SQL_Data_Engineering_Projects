# [🏗️ Data Warehouse & Mart Build: Production ETL Pipeline](dw_datawarehouse_project/)
...
- ✅ **Pipeline scope:** Built a complete ETL pipeline from raw CSVs to a star schema warehouse to analytical marts
...
**Solution:** An end-to-end ETL pipeline that extracts CSVs from cloud storage, normalizes them into a star schema warehouse (separating facts from dimensions), and creates specialized data marts optimized for specific use cases (flat queries, skill demand analysis, priority role tracking).
...
- `02_load_schema_dw.sql` – Extracts CSVs from GCS and loads them into warehouse tables
...
**SQL File:** `03_create_flat_mart.sql` – Builds a denormalized table with all dimensions joined
**Purpose:** Quick ad-hoc queries without needing joins
...
**SQL File:** `04_create_skills_mart.sql` – Builds a time-series skill demand mart
...
**SQL File:** `07_create_company_mart.sql` – Builds a company hiring trends mart (optional)
...
- **Incremental Updates:** MERGE operations for upsert patterns (INSERT, UPDATE, DELETE in a single statement)
...
- **Surrogate Keys:** Sequential ID generation using CTEs with self-joins (optional company mart build only)
