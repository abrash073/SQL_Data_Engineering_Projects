...
-- DISTINCT ON ensures we only get ONE row per job_id
SELECT DISTINCT ON (jpf.job_id)
  jpf.job_id,
  jpf.job_title_short,
...
-- If there are duplicates, order by priority_lvl to keep the most important one
ORDER BY jpf.job_id, r.priority_lvl ASC; 

-- Keep your final SELECT as it was
SELECT
    job_title_short,
    COUNT(*) AS job_count,
    MIN(priority_lvl) AS priority,
